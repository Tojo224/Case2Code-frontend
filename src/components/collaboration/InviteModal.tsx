      ...
      <div className="bg-white rounded-2xl shadow-2xl border border-slate-200 w-full max-w-lg overflow-hidden">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 bg-slate-50/50">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h2 className="text-base font-bold text-slate-800">Colaboradores del Proyecto</h2>
              <p className="text-xs text-slate-500 truncate max-w-xs">{currentDoc.name}</p>
            </div>
          </div>
          <button
        ...
        <div className="p-6 space-y-6 max-h-[75vh] overflow-y-auto">
          ...
          <div>
            ...
            <div className="flex gap-2">
              <input
                ...
                className="flex-1 px-3.5 py-2 text-sm border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
              />
              <select
                value={role}
                onChange={(e) => setRole(e.target.value as 'EDITOR' | 'VIEWER')}
                className="px-3 py-2 text-xs font-semibold bg-slate-100 border border-slate-300 rounded-lg text-slate-700 focus:outline-none focus:ring-2 focus:ring-indigo-500"
              >
          ...
          {availableDemoUsers.length > 0 && (
            <div className="p-3.5 bg-indigo-50/60 rounded-xl border border-indigo-100">
              <span className="block text-[11px] font-bold text-indigo-900 uppercase tracking-wide mb-2">
              ...
              <div className="flex flex-wrap gap-2">
                {availableDemoUsers.map((d) => (
                  <button
                    ...
                    className="flex items-center gap-1.5 px-2.5 py-1 text-xs bg-white hover:bg-indigo-100 border border-indigo-200 text-indigo-800 rounded-lg font-medium transition-colors shadow-2xs"
                  >
                    <span
          ...
          <div className="flex items-center justify-between p-3 bg-slate-50 rounded-xl border border-slate-200">
            <div>
              <p className="text-xs font-semibold text-slate-700">Enlace directo al proyecto</p>
              <p className="text-[11px] text-slate-400">Cualquier miembro invitado puede entrar con este enlace</p>
            </div>
            <button
              onClick={handleCopyLink}
              className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold text-slate-700 hover:text-slate-900 bg-white hover:bg-slate-100 border border-slate-300 rounded-lg transition-colors shadow-2xs"
            >
