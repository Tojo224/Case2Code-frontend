...
export const CaseAssistantChat: React.FC = () => {
  ...
  return (
    <aside className="fixed right-4 top-16 bottom-4 w-96 bg-white border border-slate-200/90 rounded-2xl shadow-2xl flex flex-col z-40 overflow-hidden animate-in slide-in-from-right duration-200">
      {/* Header */}
        ...
        <div className="flex items-center space-x-2.5">
          ...
          <div>
            <div className="flex items-center space-x-1.5">
              <h3 className="font-bold text-sm tracking-tight">Asistente CASE</h3>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3.5 bg-slate-50/50">
                ...
                <div className="mt-2.5 pt-2 border-t border-slate-100 space-y-1">
                  ...
                  <div className="flex flex-wrap gap-1 mt-1">
                    {msg.executedCommands.map((cmd, idx) => (
                      <span
                        key={idx}
                        className="bg-emerald-50 text-emerald-700 border border-emerald-200/60 rounded px-1.5 py-0.5 text-[10px] font-mono flex items-center space-x-1"
                      >
        ...
        {isLoading && (
          <div className="flex items-center space-x-2 text-xs text-slate-500 bg-white border border-slate-200 rounded-xl p-3 max-w-[70%]">
      ...
      {messages.length <= 2 && (
        <div className="px-3 py-2 bg-slate-100/70 border-t border-slate-200">
          ...
          <div className="flex flex-wrap gap-1.5">
            {quickPrompts.map((qp, idx) => (
              <button
                ...
                className="text-[11px] bg-white hover:bg-sky-50 hover:text-sky-700 hover:border-sky-300 border border-slate-200 rounded-lg px-2 py-1 text-slate-600 text-left transition shadow-2xs"
              >
                {qp}
              </button>
            ))}
      ...
      <div className="p-3 bg-white border-t border-slate-200">
        {isListening && (
          <div className="mb-2 flex items-center justify-between bg-red-50 border border-red-200 px-3 py-1.5 rounded-lg text-xs text-red-600 animate-pulse">
        ...
        <div className="flex items-center space-x-1.5">
          <div className="relative flex-1">
            <input
              ...
              className="w-full text-xs bg-slate-50 border border-slate-200 rounded-xl px-3 py-2.5 pr-9 focus:outline-none focus:ring-2 focus:ring-sky-500 focus:bg-white transition"
            />
            {speechSupported && (
              <button
                type="button"
